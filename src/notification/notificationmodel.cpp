...
void NotificationModel::markUpToAsRead(const QString &notificationId)
{
    if (m_notifications.isEmpty() || loading()) {
        return;
    }

    // Only update the marker if this notification is newer than the current lastReadId
    // Mastodon IDs are lexicographically comparable (snowflake IDs)
    if (!m_lastReadId.isEmpty() && notificationId <= m_lastReadId) {
        return;
    }
    ...
    for (int i = 0; i < m_notifications.size(); i++) {
        ...
        if (notification->unread()) {
            notification->setUnread(false);
            markedCount++;
            Q_EMIT dataChanged(AbstractTimelineModel::index(i, 0), AbstractTimelineModel::index(i, 0), {UnreadRole});
        }
    ...
    m_lastReadId = notificationId;
    Q_EMIT readMarkerChanged();

    // Save the marker on the server
    m_account->saveTimelinePosition(QStringLiteral("notifications"), m_lastReadId);

    // Update the unread count
    if (markedCount > 0 && m_account->unreadNotificationsCount() > 0) {
        int newCount = m_account->unreadNotificationsCount() - markedCount;
        if (newCount < 0) {
            newCount = 0;
        }
        // We can't set the count directly, so just reset it and let the next poll update
        m_account->resetUnreadNotificationsCount();
    }
}
