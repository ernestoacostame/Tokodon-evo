...
Kirigami.ScrollablePage {
    ...
    property int mentionStartIndex: -1
    property string currentMentionText: ""

    SearchModel {
        id: followedAccountsSearchModel
    }

    SearchModel {
        id: globalAccountsSearchModel
    }
    ...
    QQC2.Popup {
        ...
        parent: textArea
        y: textArea.cursorRectangle.y + textArea.cursorRectangle.height + Kirigami.Units.smallSpacing
        x: textArea.cursorRectangle.x

        width: Kirigami.Units.gridUnit * 16
        height: Math.min(contentLayout.implicitHeight + Kirigami.Units.largeSpacing, Kirigami.Units.gridUnit * 15)
        ...
        contentItem: QQC2.ScrollView {
            ...
            ColumnLayout {
                ...
                Repeater {
                    model: followedAccountsSearchModel
                    delegate: QQC2.ItemDelegate {
                        Layout.fillWidth: true
                        
                        onClicked: root.insertMention(authorIdentity.account)

                        contentItem: RowLayout {
                            spacing: Kirigami.Units.smallSpacing
                            Components.Avatar {
                                source: authorIdentity ? authorIdentity.avatarUrl : ""
                                name: authorIdentity ? authorIdentity.displayName : ""
                            ...
                            ColumnLayout {
                                spacing: 0
                                QQC2.Label {
                                    text: authorIdentity ? authorIdentity.displayName : ""
                                    font.bold: true
                                    elide: Text.ElideRight
                                    Layout.fillWidth: true
                                }
                                QQC2.Label {
                                    text: authorIdentity ? ("@" + authorIdentity.account) : ""
                ...
                Kirigami.Separator {
                    Layout.fillWidth: true
                    visible: followedAccountsSearchModel.rowCount > 0 && globalAccountsSearchModel.rowCount > 0
                }

                Repeater {
                    model: globalAccountsSearchModel
                    delegate: QQC2.ItemDelegate {
                        Layout.fillWidth: true
    ...
    function insertMention(acct) {
        if (root.mentionStartIndex !== -1) {
            let before = textArea.text.substring(0, root.mentionStartIndex);
            let after = textArea.text.substring(textArea.cursorPosition);
            textArea.text = before + "@" + acct + " " + after;
            textArea.cursorPosition = before.length + acct.length + 2;
            root.mentionStartIndex = -1;
            root.currentMentionText = "";
            mentionsPopup.close();
            textArea.forceActiveFocus();
        }
    }
    ...
    Kirigami.FlexColumn {
        ...
        QQC2.TextArea {
            ...
            function checkMentions() {
                let cursor = textArea.cursorPosition;
                let textToCursor = textArea.text.substring(0, cursor);
                let match = textToCursor.match(/@([\w\d_.-]+)$/);
                if (match && match[1].length >= 2) {
                    let newText = match[1];
                    if (root.currentMentionText !== newText || root.mentionStartIndex !== match.index) {
                        root.mentionStartIndex = match.index;
                        root.currentMentionText = newText;
                        followedAccountsSearchModel.search(root.currentMentionText, "accounts", true);
                        globalAccountsSearchModel.search(root.currentMentionText, "accounts", false);
                    }
                    if (!mentionsPopup.opened) {
                        mentionsPopup.open();
                    }
                } else {
                    root.mentionStartIndex = -1;
