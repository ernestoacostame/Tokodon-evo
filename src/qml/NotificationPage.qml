...
Kirigami.ScrollablePage {
    ...
    // Auto-mark notifications as read when the user views the page
    Timer {
        id: markAsReadTimer
        interval: 2000
        repeat: false
        running: !root.loading && listview.count > 0
        onTriggered: {
            if (root.currentModel.markAllNotificationsAsRead) {
                root.currentModel.markAllNotificationsAsRead();
            }
        }
    }

    // Also mark as read when user scrolls to the top
    Connections {
        target: listview
        function onAtYBeginningChanged(): void {
            if (listview.atYBeginning && !root.loading && listview.count > 0) {
