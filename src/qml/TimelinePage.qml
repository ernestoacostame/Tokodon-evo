...
Kirigami.ScrollablePage {
    ...
    // Periodic auto-refresh timer — silently checks for new posts without reloading
    Timer {
        id: autoRefreshTimer
        interval: Config.autoRefreshInterval * 1000 // seconds to ms
        repeat: true
        running: Config.autoRefreshInterval > 0 && root.isCurrentPage
        onTriggered: {
            if (root.model.checkForNewPosts) {
                root.model.checkForNewPosts();
            }
            if (Config.autoScrollOnRefresh && root.model.newPostsAvailable) {
                root.returnToTop();
            }
        }
    }
    ...
    function returnToTop(): void {
        if (root.model.clearNewPostsAvailable) {
            root.model.clearNewPostsAvailable();
        }
        root.model.refresh();
    ...
    footer: Kirigami.InlineMessage {
        ...
        text: i18n("New posts available")
        visible: root.model.newPostsAvailable ?? false
        position: Kirigami.InlineMessage.Position.Header
        showCloseButton: true
        onVisibleChanged: if (!visible && root.model.clearNewPostsAvailable) root.model.clearNewPostsAvailable()

        actions: Kirigami.Action {
            icon.name: "arrow-up-symbolic"
            text: i18nc("@action:button", "Go to Top")
            onTriggered: root.returnToTop()
        }
    }
